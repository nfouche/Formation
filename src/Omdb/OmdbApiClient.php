...
{
    public const HOST = 'http://www.omdbapi.com';

    /** @var string Api key for OMDb api. */
    private $token;

    /** @var string[] Strict parameters for request on OMDb api. */
    ...
    /** @var string[] Optional parameters to complete a request. */
    ...
    public function __construct(HttpClientInterface $httpClient, string $omdbToken)
    {
    ...
    private function requestBy(array $queryParameters): array
    {
        ...
        $response = $this->httpClient
            ->request('GET', self::HOST, ['query' => $queryParameters])
            ->toArray()
        ;
